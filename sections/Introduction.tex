...
\stitle{Large data size of trajectories} The trajectory data size is extremely huge.
...
%Visually, both are quite different from the visualization result of the whole data set, as illustrated in Figure~\ref{fig:teaser} C.
...
In this work, we propose a visual fidelity guaranteed sampling approach ($\vats{}$) for the line-based trajectory visualization problem,
...
Intuitively, the visual clutter issue in Figure~\ref{fig:teaser}(E) is \UC{almost} removed in Figure~\ref{fig:teaser}(F).
