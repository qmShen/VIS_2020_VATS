...
Rendering refers to the use of the hardware device (e.g., GPU) in the \ZW{generation} of \ZW{visualizations}.
...
\QM{Sampling techniques are de-facto standards} for large-scale data analysis in both database and visualization community.
...
Fig.~\ref{fig:teaser}(D) and (E) show the visualization results of our proposal $\avats{}$ on \pt{} taxi trajectory dataset with \QM{the} sampling rate $0.1\%$ and $1\%$, respectively.
...
Fig.~\ref{fig:teaser}(F) is the returning result of our proposal which colors the trajectories \BQM{(with different representativeness)}\QM{according to the trajectory representativeness}.
It has the same input parameters of Fig.~\ref{fig:teaser}(E).
Intuitively, the visual clutter in Fig.~\ref{fig:teaser}(A) and (E) \QM{are alleviated shown as} Fig.~\ref{fig:teaser}(F).
...
\squishlist
  \item We formulate the visual fidelity guaranteed sampling problem for large trajectory data visualization, and prove it is \BQM{(NP hard)}\QM{NP-hard} in Section~\ref{sec:pro}.
  ...
  \item We propose an advanced approach to further enhance the effectiveness of our approximate algorithm, which \BQM{(address)}\QM{address} the visual clutter by introducing perception tolerance parameter,
  and encodes the representativeness of each \BQM{(road)}\QM{trajectory} by different colors in Section~\ref{sec:aa}.
  \item We conduct extensive experiments on real-world trajectory datasets to demonstrate the superiority of our proposals in Section~\ref{sec:exp}. Especially, we conduct qualitative user studies to show \BQM{(their)} \QM{the} effectiveness on three real-world applications.
\squishend
%\end{itemize}
