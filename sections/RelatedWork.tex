...
%\BQM{(With the recent advance of location-acquisition technology, the size of available trajectory dataset becomes extremely huge.)}
...
{Aggregating-based optimization techniques~\cite{zeng2013visualizing,von2015mobilitygraphs} in the middle-layer support interactive visual analysis for large-scale data by
	reducing the number of rendering data.}
%\QM{Aggregating-based techniques pre-process raw data with aggregation techniques (e.g., clustering) in the middle-layer, yield fewer rendering objects for interactive visual analytics.}
