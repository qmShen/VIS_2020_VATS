...
\stitle{Example} Given the Porto trajectory dataset, it has 2.39 millions trajectories, the maximum length in it is 3,490.
It takes 2,430 seconds to return a subset $\oR$ with sampling rate $1\%$.
\TB{Obviously, it is impractical for interactive trajectory explorations.}
...
With the help of submodularity property in Lemma~\ref{lem:submodular}, it reduces many unnecessary trajectory contribution value computations.
...
\TB{To exemplify, Algorithm~\ref{alg:greedy} costs 2430 seconds to return the results with sampling rate $1\%$ on Porto taxi trajectory dataset.
However, it only needs 3.2 seconds via lazy computing manner.}
...
\stitle{Trajectory data distribution} Considering Porto trajectory dataset, Figure~\ref{fig:delta}(A) is the visualization result of $\vats$ with sampling ratio $1\%$.
...
\stitle{Human perception ability} Intuitively, it is much easier for humans to distinguish the difference between sparse regions rather than dense regions in Figure~\ref{fig:delta}(A) and (B).
...
Fortunately, we can slightly revise Algorithm~\ref{alg:greedy} to incorporate the perception tolerance parameter $\delta$ in advance approach $\avats$, as shown in Algorithm~\ref{alg:plus}.
...
Inherently, the selection process in $\avats$ embeds the representativeness of each trajectory in the result set $\oR$.
