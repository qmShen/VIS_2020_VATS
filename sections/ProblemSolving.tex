...
Due to the hardness of the Problem~\ref{prob:def}, we first introduce a straight-forward solution, i.e., uniform random sampling $\rand$, for it in Section~\ref{sec:random}.
Then, we propose a visual fidelity guaranteed sampling approach $\vats$ in Section~\ref{sec:greedy}.
...
As the pseudocode in Algorithm~\ref{alg:rand} shown, it selects $k$ trajectories from $\D$ randomly and stores them in $\oR$.
We last render these selected trajectories in $\oR$ as the visualization result.
...
%has good performance. However, it
...
%The visual fidelity guaranteed sampling algorithm .
...
\begin{figure}
 ...
 \end{tabular}
 \vspace{-3mm}
 \caption{Lazy computing manner via the submodularity in Lemma~\ref{lem:submodular}.} \label{fig:heap}
 \vspace{-6mm}
\end{figure}
...
\begin{figure}[t]
	...
	\vspace{-3mm}
	\caption{Advance approach $\avats$ with \pt{}, sampling rate $\alpha = 0.5\%$.}
	\label{fig:delta}
	 \vspace{-3mm}
\end{figure}
