...
%\newcommand{\vats}{\mathsf{VFGTS}}
%\newcommand{\avats}{\mathsf{VFGTS}^{+}}
...
%However, it is very challenging due to (i) large trajectory data size, (ii) limited rendering capability of graphics device, and (iii) visual clutter issue.
...
%We then prove that it is NP-hardness to select a sized-$k$ subset trajectories with minimal visual fidelity loss.
...
%Moreover, we improve the visual effectiveness of $\vats$ by taking human perception ability and visual clutter issue into consideration.
...
%In addition, qualitative user studies further illustrate the superiority of our approach in various applications, e.g., traffic flow detection, reachable route discovery.
% Trajectory visualization is a core subroutine in many smart city applications, e.g., traffic management, and route recommendation.
%\QM{submit version}Visualizing large-scale trajectories is the core subroutine in many smart city applications, e.g., traffic management, route recommendation. However, it is very challenging as (i) the large trajectory data size, (ii) the limited rendering capability of graphics device, and (iii) visual clutter in large data visualization. In this work, we propose a visual fidelity guaranteed sampling approach for the large-scale line-based trajectory visualization. Specifically, we first define a novel fidelity loss function to capture the visual difference between two visualization results. We then prove that it is NP-hardness to select a sized-$k$ subset of trajectories with minimal visual fidelity loss. Next, we devise an approximate algorithm with a suite of optimization techniques, which returns visual fidelity quality guaranteed visualization result efficiently. Moreover, we improve the visual effectiveness of our proposal by taking human perception ability and visual clutter into consideration. We conduct extensive experimental studies to demonstrate the effectiveness of our proposal on real-world trajectory datasets. In addition, qualitative user studies further illustrate the superiority of our approach in various applications, e.g., region center identification, reachable route inspection, traffic flow detection.
...
In this work, we propose a visual fidelity guaranteed sampling algorithm for line-based visualization of large trajectory data.
...
\keywords{Trajectory visualization, visual fidelity, data sampling}


\teaser{
  \centering
  \includegraphics[width=\linewidth]{Teaser.pdf}
  \caption{Comparison of random sampling and $\avats$. (i) A is the visualization of the whole Porto taxi trajectory dataset at zoom level 11.
   B and C are visualizations of random sampling with sampling rate $0.001$ and $0.01$, respectively.
   D, E, F are visualizations of our proposed $\avats$ with different parameters.
   (II) G, H, and I are visualizations of the whole dataset, random sampling with sampling rate 0.01, and $\avats$ with sampling rate 0.01 at zoom level 15, respectively.}
	\label{fig:teaser}
}
